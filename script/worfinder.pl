...
Optional:
 ...
 -mersize :i                 => Size of kmer for RNAi design (defaults to 21).
...
my $query = "SELECT * FROM genome where filename is not null  ";
...
while (<IN>) {
    ...
    for ( my $i = 0 ; $i < @dsRNA - $mer_size ; $i++ ) {
      my @off = @dsRNA[ $i .. $i + $mer_size -1 ];
      $offTarget = join "", @off;
...
#        print "$database";
        ...
        system(
"bowtie  -f $database_dir/$database $out_dir/bowtiein.fasta --quiet -p 4 -v $mm -a >$out_dir/$dsRNA_Name"
              . "_$database"
