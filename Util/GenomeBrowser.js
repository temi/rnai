Ext.define('Util.GenomeBrowser', {
  extend : 'Ext.panel.Panel',
  alias : 'widget.genomebrowser',
  requires:['RNAi.store.CanvasXpress'],
  title: null,
  ...
  canvasOptions:null,
  /**
   * field name that will get canvas config.
   */
  canvasField: 'canvas',
  /**
  ...
  initComponent : function() {
    ...
    this.store.addListener('beforeload',this.loadOn, this );
    this.store.addListener('load', this.loadOff, this );
  ...
  onload:function(store , records, success){
    if( success ){
      ...
      this.canvasData = record.get( this.canvasField );
      this.canvasOptions = this.options;
      
      if( this.canvasXpress ){
        this.canvasXpress.data = this.canvasData;
        this.canvasXpress.redraw();
      }else {
        this.loadCanvas();
      }
    }
  ...
  renderCanvas : function() {
    ...
    // if( this.canvasData && this.canvasData.tracks[0].data[0].sequence ){
	 ...
	 if( this.canvasXpress ){
            cToolbar = Ext.get( this.canvasId + '-cX-ToolbarConfigurator');
            cToolbar && cToolbar.hide();
         } 
      }
  },
